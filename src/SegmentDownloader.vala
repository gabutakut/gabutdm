...
namespace Gabut {
    public class SegmentDownloader : GLib.Object {
        public signal void finished(int index, bool ok);
        public signal void forcewait(int index);
        public signal void progress(int index, int64 current, int64 total);
        public signal void status_changed(int index, int newstatus);
        ...
        private void download_with_resume() throws Error {
            ...
            if (file.query_exists() && !control_file.query_exists()) {
                ...
                if (ffread.validate_path(file.get_path()) == 0) {
                    ...
                    status_changed(index, StatusMode.COMPLETE);
                    completed = true;
                    success = true;
                    processing = false;
            ...
            if (control_file.query_exists()) {
                ...
                if (uri != null && uri.get_scheme() != null && uri.get_host() != null) {
                    ...
                    msg_get.request_headers.append("Referer", "%s://%s/".printf(uri.get_scheme(), uri.get_host()));
                }
        ...
        private void download_parse (GLib.File file) throws Error {
            ...
            GLib.Timeout.add(500, () => {
                ...
                if (total_size > 0) {
                    ...
                    progress(index, current_dl, total_size);
                }
        ...
        private void download_part_sync(Json.Object part_obj) throws GLib.Error {
            ...
            if (!success && !cancellable.is_cancelled()) {
                throw new GLib.IOError.FAILED("Max retries exceeded for part %lld".printf(id));
            }
        }
        ...
        private void perform_part_download_sync(Json.Object part_obj) throws GLib.Error {
            ...
            if (headerdm != "NOTSET") {
                msg.request_headers.append ("Cookie", headerdm);
            }
            ...
            if (uri != null && uri.get_scheme() != null && uri.get_host() != null) {
                ...
                msg.request_headers.append("Referer", "%s://%s/".printf(uri.get_scheme(), uri.get_host()));
            }
